...
Panel {
  ...
  // ---- Advanced task edit (Shift+Q). A separate, full-field editor from
  //      the lightweight inline title-only edit above ("e") — that stays
  //      exactly as it is. "" = closed; otherwise the id of the task being
  //      edited (keyed by id, not index, since a background refresh can
  //      reorder root.tasks while the overlay is open).
  property string advancedEditTaskId: ""
  property string aeTitleDraft: ""
  property int aePriorityDraft: 1
  property string aeDueDraft: ""
  property string aeOriginalDueString: ""
  property string aeProjectId: ""
  property string aeOriginalProjectId: ""
  property var aeLabelsDraft: []
  property var aeOriginalLabels: []
  property bool aeSaving: false
  property string aeError: ""
  property int aePendingCalls: 0

  // Projects/labels are fetched once per panel lifetime (on first advanced
  // edit) and cached — they change rarely enough that refetching on every
  // open would just be latency for no benefit.
  property var projectsList: []
  property bool projectsLoading: false
  property bool projectsLoaded: false
  property var labelsList: []
  property bool labelsLoading: false
  ...
  // ---- Advanced task edit (Shift+Q) -----------------------------------
  //      Title, priority, due (natural language, same parser as quick-add),
  //      project, labels — everything the lightweight "e" edit above
  //      doesn't cover. Project moves go through a separate endpoint
  //      (/tasks/{id}/move) — Update Task itself has no project_id field.
  function startAdvancedEdit() {
    if (root.selectedTaskIndex < 0 || root.selectedTaskIndex >= root.tasks.length) return
    var task = root.tasks[root.selectedTaskIndex]
    if (!task || root.completingTaskIds.indexOf(task.id) !== -1) return
    root.aeTitleDraft = task.content || ""
    root.aePriorityDraft = typeof task.priority === "number" ? task.priority : 1
    var dueString = (task.due && task.due.string) ? task.due.string : ""
    root.aeDueDraft = dueString
    root.aeOriginalDueString = dueString
    root.aeProjectId = task.project_id || ""
    root.aeOriginalProjectId = task.project_id || ""
    root.aeLabelsDraft = (task.labels || []).slice()
    root.aeOriginalLabels = (task.labels || []).slice()
    root.aeError = ""
    root.aeSaving = false
    root.advancedEditTaskId = task.id
    // Dropdown/MultiSelect sever their declarative `value`/`values` binding
    // the moment the user picks something (assigning to a bound property
    // severs it — same gotcha as TaskRow.editField below). Since both
    // components persist across opens/closes (visible:false doesn't
    // destroy them), a second edit needs this explicit re-sync or it'd
    // keep showing the previous task's selection.
    projectDropdown.value = root.aeProjectId
    labelsMultiSelect.values = root.aeLabelsDraft
    root.ensureProjectsLoaded()
    root.ensureLabelsLoaded()
    Qt.callLater(function() {
      if (root.advancedEditTaskId === task.id) aeTitleField.forceActiveFocus()
    })
  }

  function cancelAdvancedEdit() {
    if (root.aeSaving) return
    root.advancedEditTaskId = ""
    root.aeError = ""
    Qt.callLater(function() { keyCatcher.forceActiveFocus() })
  }

  function ensureProjectsLoaded() {
    if (root.projectsLoaded || root.projectsLoading) return
    root.projectsLoading = true
    projectsProc.command = ["curl", "-fsS", "--max-time", "10",
      "-H", "Authorization: Bearer " + root.apiToken,
      root.apiBase + "/projects?limit=200"]
    projectsProc.running = true
  }

  function ensureLabelsLoaded() {
    if (root.labelsLoaded || root.labelsLoading) return
    root.labelsLoading = true
    labelsProc.command = ["curl", "-fsS", "--max-time", "10",
      "-H", "Authorization: Bearer " + root.apiToken,
      root.apiBase + "/labels?limit=200"]
    labelsProc.running = true
  }

  // ---- Advanced edit keyboard nav. Same explicit-chain approach as
  //      settingsFocusChain() and for the same reason (PanelKeyCatcher's
  //      BeforeItem interception only works while it holds real focus).
  //      Project/Labels use small focusable proxy Items rather than the
  //      Dropdown/MultiSelect components themselves — those don't expose
  //      their internal focusable trigger for an external forceActiveFocus()
  //      to land on, so the proxy stands in for "this field has the chain's
  //      cursor" and drives the real component's own `hasCursor` visual.
  function advancedEditFocusChain() {
    var chain = [aeTitleField, aeP1Button, aeP2Button, aeP3Button, aeP4Button,
      aeDueField, aeClearDueButton, aeProjectProxy, aeLabelsProxy,
      aeSaveButton, aeCancelButton]
    return chain.filter(function(item) { return item && item.enabled !== false })
  }

  function moveAdvancedFocus(direction) {
    var chain = root.advancedEditFocusChain()
    if (chain.length === 0) return
    var currentIndex = -1
    for (var i = 0; i < chain.length; i++) {
      if (chain[i] && chain[i].activeFocus) { currentIndex = i; break }
    ...
    var next = currentIndex === -1 ? 0 : (currentIndex + direction + chain.length) % chain.length
    if (chain[next]) chain[next].forceActiveFocus()
  }

  function activateFocusedAdvancedControl() {
    var chain = root.advancedEditFocusChain()
    for (var i = 0; i < chain.length; i++) {
      var item = chain[i]
      if (!item || !item.activeFocus) continue
      if (typeof item.activate === "function") { item.activate(); return }
      if (typeof item.clicked === "function") { item.clicked(); return }
      return
    }
  }
  ...
  function commitAdvancedEdit() {
    if (root.aeSaving || root.advancedEditTaskId === "") return
    var taskId = root.advancedEditTaskId
    var index = -1
    for (var i = 0; i < root.tasks.length; i++) {
      if (root.tasks[i] && root.tasks[i].id === taskId) { index = i; break }
    }
    if (index === -1) { root.cancelAdvancedEdit(); return }

    var title = Model.safeTrim(root.aeTitleDraft)
    if (title === "") { root.aeError = "Title can't be empty."; return }

    var original = root.tasks[index]
    var updateBody = {}
    if (title !== original.content) updateBody.content = title
    if (root.aePriorityDraft !== original.priority) updateBody.priority = root.aePriorityDraft
    if (!root.labelsEqual(root.aeLabelsDraft, root.aeOriginalLabels)) updateBody.labels = root.aeLabelsDraft
    var dueTrimmed = Model.safeTrim(root.aeDueDraft)
    var dueChanged = dueTrimmed !== Model.safeTrim(root.aeOriginalDueString)
    if (dueChanged) updateBody.due_string = dueTrimmed === "" ? "no date" : dueTrimmed
    var projectChanged = root.aeProjectId !== "" && root.aeProjectId !== root.aeOriginalProjectId

    if (Object.keys(updateBody).length === 0 && !projectChanged) {
      root.cancelAdvancedEdit()
      return
    }
    ...
    // Optimistic local update — same pattern as commitEditTask()/complete/
    // delete above: apply now, reconcile on failure via the shared error
    // path rather than hand-rolling a revert.
    var updated = {}
    for (var key in original) updated[key] = original[key]
    if (updateBody.content !== undefined) updated.content = updateBody.content
    if (updateBody.priority !== undefined) updated.priority = updateBody.priority
    if (updateBody.labels !== undefined) updated.labels = updateBody.labels
    if (dueChanged) {
      updated.due = dueTrimmed === "" ? null : { date: original.due ? original.due.date : "", string: dueTrimmed }
    }
    if (projectChanged) updated.project_id = root.aeProjectId
    var nextTasks = root.tasks.slice()
    nextTasks[index] = updated
    root.tasks = nextTasks

    root.aePendingCalls = 0
    if (Object.keys(updateBody).length > 0) {
      root.aePendingCalls++
      advancedUpdateProc.command = ["curl", "-fsS", "--max-time", "10", "-X", "POST",
        "-H", "Authorization: Bearer " + root.apiToken,
        "-H", "Content-Type: application/json",
        "-d", JSON.stringify(updateBody),
        root.apiBase + "/tasks/" + encodeURIComponent(taskId)]
      advancedUpdateProc.running = true
    }
    if (projectChanged) {
      root.aePendingCalls++
      advancedMoveProc.command = ["curl", "-fsS", "--max-time", "10", "-X", "POST",
        "-H", "Authorization: Bearer " + root.apiToken,
        "-H", "Content-Type: application/json",
        "-d", JSON.stringify({ project_id: root.aeProjectId }),
        root.apiBase + "/tasks/" + encodeURIComponent(taskId) + "/move"]
      advancedMoveProc.running = true
    }
  }

  function handleAdvancedEditStepDone(failed, message) {
    root.aePendingCalls = Math.max(0, root.aePendingCalls - 1)
    if (failed) {
      root.aeSaving = false
      root.aeError = message
      return
    }
    if (root.aePendingCalls > 0) return
    root.aeSaving = false
    root.advancedEditTaskId = ""
    root.refresh()
    Qt.callLater(function() { keyCatcher.forceActiveFocus() })
  ...
  // ---- Advanced task edit: project/label lists (fetched once, cached)
  //      and the two possible save calls (Update Task, Move Task).
  Process {
    id: projectsProc
    stdout: StdioCollector {
      id: projectsOut
      ...
      onStreamFinished: {
        ...
        try {
          var parsed = JSON.parse(raw)
          root.projectsList = Model.projectOptions((parsed && parsed.results) ? parsed.results : [])
        } catch (e) { /* leave list empty — dropdown just has nothing to offer */ }
      }
    }
    stderr: StdioCollector {
      id: projectsErr
    ...
    onExited: function(exitCode) {
      root.projectsLoading = false
      root.projectsLoaded = true
      if (exitCode !== 0 && root.aeError === "") root.aeError = Model.errorMessageForExit(exitCode, projectsErr.text)
    }
  }
  ...
  Process {
    ...
    stdout: StdioCollector {
      ...
      waitForEnd: true
      onStreamFinished: {
        root.labelsLoading = false
        root.labelsLoaded = true
        var raw = String(text || "").trim()
        if (raw === "") return
        try {
          var parsed = JSON.parse(raw)
          root.labelsList = Model.labelOptions((parsed && parsed.results) ? parsed.results : [])
        } catch (e) { /* leave list empty — multiselect just has nothing to offer */ }
      }
    }
    stderr: StdioCollector {
      id: labelsErr
    ...
    onExited: function(exitCode) {
      root.labelsLoading = false
      root.labelsLoaded = true
      if (exitCode !== 0 && root.aeError === "") root.aeError = Model.errorMessageForExit(exitCode, labelsErr.text)
    }
  }

  Process {
    id: advancedUpdateProc
    stderr: StdioCollector {
      id: advancedUpdateErr
      waitForEnd: true
    }
    onExited: function(exitCode) {
      root.handleAdvancedEditStepDone(exitCode !== 0, Model.errorMessageForExit(exitCode, advancedUpdateErr.text))
    }
  }
  ...
  Process {
    ...
    onExited: function(exitCode) {
      root.handleAdvancedEditStepDone(exitCode !== 0, Model.errorMessageForExit(exitCode, advancedMoveErr.text))
    }
  }
  ...
  KeyboardPanel {
    ...
    PanelKeyCatcher {
      ...
      anchors.fill: parent
      clip: true
      blocked: tokenField.activeFocus || filterField.activeFocus || quickAddField.activeFocus || root.recordingKeybind || confirmDialog.opened || root.editingTaskIndex !== -1 || root.helpOpen || root.advancedEditTaskId !== ""
      ...
      onTextKey: function(t) {
        ...
        if (t === "q") { quickAddField.forceActiveFocus(); return }
        // Shift+Q — advanced edit (title/priority/due/project/labels).
        // Distinct from plain "q" (focus quick-add). "e"/"E" stay exactly
        // as they were: the lightweight title-only inline edit.
        if (t === "Q") { root.startAdvancedEdit(); return }
      ...
      // Advanced task edit (Shift+Q). Declared last so it paints above
      // everything else, including the confirm dialog and shortcuts help
      // (the two can't actually coexist — keyCatcher's `blocked` guard
      // means Shift+Q can't fire while either of those is open, and vice
      // versa — but painter's order is kept consistent regardless).
      Item {
        id: advancedEditOverlay
        anchors.fill: parent
        visible: root.advancedEditTaskId !== ""
        ...
        Rectangle {
          ...
          MouseArea {
            anchors.fill: parent
            onClicked: root.cancelAdvancedEdit()
          }
          ...
          BorderSurface {
            ...
            width: Math.min(parent.width - Style.space(24), Style.space(340))
            height: Math.min(parent.height - Style.space(24),
              aeCard.contentTopInset + aeCard.contentBottomInset + aeColumn.implicitHeight + Style.space(8))
            color: Color.popups.background
            borderSpec: Border.flat(Color.accent, Style.normalBorderWidth)
            radius: Style.cornerRadius
            padding: Style.space(16)
            ...
            Item {
              anchors.fill: parent
              anchors.topMargin: aeCard.contentTopInset
              anchors.rightMargin: aeCard.contentRightInset
              anchors.bottomMargin: aeCard.contentBottomInset
              anchors.leftMargin: aeCard.contentLeftInset

              Flickable {
                anchors.fill: parent
                contentWidth: width
                contentHeight: aeColumn.implicitHeight
                clip: true
                boundsBehavior: Flickable.StopAtBounds
                interactive: contentHeight > height
                ...
                Column {
                  ...
                  TextField {
                    id: aeTitleField
                    width: parent.width
                    enabled: !root.aeSaving
                    text: root.aeTitleDraft
                    onTextChanged: root.aeTitleDraft = text
                    onAccepted: root.commitAdvancedEdit()
                    Keys.onEscapePressed: root.cancelAdvancedEdit()
                    Keys.onPressed: function(event) {
                      if (event.key === Qt.Key_Tab) { root.moveAdvancedFocus(1); event.accepted = true }
                      else if (event.key === Qt.Key_Backtab) { root.moveAdvancedFocus(-1); event.accepted = true }
                    }
                  }
                  ...
                  Column {
                    ...
                    Row {
                      ...
                      NavButton {
                        ...
                        text: "P1"
                        enabled: !root.aeSaving
                        selected: root.aePriorityDraft === 4
                        onClicked: root.aePriorityDraft = 4
                      }
                      NavButton {
                        id: aeP2Button
                        text: "P2"
                        enabled: !root.aeSaving
                        selected: root.aePriorityDraft === 3
                        onClicked: root.aePriorityDraft = 3
                      }
                      NavButton {
                        id: aeP3Button
                        text: "P3"
                        enabled: !root.aeSaving
                        selected: root.aePriorityDraft === 2
                        onClicked: root.aePriorityDraft = 2
                      }
                      NavButton {
                        id: aeP4Button
                        text: "P4"
                        enabled: !root.aeSaving
                        selected: root.aePriorityDraft === 1
                        onClicked: root.aePriorityDraft = 1
                      }
                    }
                  }

                  // ---- Due date/time (one natural-language field, same
                  //      parser Quick Add already uses — no separate date
                  //      and time widgets to keep in sync).
                  Column {
                    width: parent.width
                    spacing: Style.spacing.xs
                    ...
                    TextField {
                      ...
                      width: parent.width
                      enabled: !root.aeSaving
                      placeholderText: "e.g. tomorrow at 5pm, every Monday"
                      text: root.aeDueDraft
                      onTextChanged: root.aeDueDraft = text
                      onAccepted: root.commitAdvancedEdit()
                      Keys.onEscapePressed: root.cancelAdvancedEdit()
                      Keys.onPressed: function(event) {
                        if (event.key === Qt.Key_Tab) { root.moveAdvancedFocus(1); event.accepted = true }
                    ...
                    NavButton {
                      id: aeClearDueButton
                      text: "Clear due date"
                      enabled: !root.aeSaving && root.aeDueDraft !== ""
                      onClicked: root.aeDueDraft = ""
                    }
                  }
                  ...
                  Column {
                    ...
                    Dropdown {
                      ...
                      showLabel: false
                      enabled: !root.aeSaving
                      options: root.projectsList
                      value: root.aeProjectId
                      foreground: root.contentForeground
                      background: Color.popups.background
                      accent: Color.accent
                      fontFamily: root.contentFontFamily
                      hasCursor: aeProjectProxy.activeFocus
                      onChanged: function(v) { root.aeProjectId = v }
                    }
                    ...
                    Item {
                      ...
                      height: 1
                      focus: false
                      function activate() { projectDropdown.open() }
                      Keys.onPressed: function(event) {
                        if (event.key === Qt.Key_Tab) { root.moveAdvancedFocus(1); event.accepted = true }
                        else if (event.key === Qt.Key_Backtab) { root.moveAdvancedFocus(-1); event.accepted = true }
                        else if (event.key === Qt.Key_Return || event.key === Qt.Key_Enter
                            || event.key === Qt.Key_Space || event.key === Qt.Key_Down) {
                          projectDropdown.open(); event.accepted = true
                        }
                      }
                    }

                    Text {
                      visible: root.projectsLoading
                  ...
                  Column {
                    ...
                    MultiSelect {
                      id: labelsMultiSelect
                      width: parent.width
                      showLabel: false
                      enabled: !root.aeSaving
                      options: root.labelsList
                      values: root.aeLabelsDraft
                      foreground: root.contentForeground
                      background: Color.popups.background
                      accent: Color.accent
                      fontFamily: root.contentFontFamily
                      hasCursor: aeLabelsProxy.activeFocus
                      onChanged: function(v) { root.aeLabelsDraft = v }
                    }
                    ...
                    Item {
                      ...
                      height: 1
                      focus: false
                      function activate() { labelsMultiSelect.open() }
                      Keys.onPressed: function(event) {
                        if (event.key === Qt.Key_Tab) { root.moveAdvancedFocus(1); event.accepted = true }
                        else if (event.key === Qt.Key_Backtab) { root.moveAdvancedFocus(-1); event.accepted = true }
                        else if (event.key === Qt.Key_Return || event.key === Qt.Key_Enter
                            || event.key === Qt.Key_Space || event.key === Qt.Key_Down) {
                          labelsMultiSelect.open(); event.accepted = true
                        }
                      }
                    }

                    Text {
                      visible: root.labelsLoading
                      text: "Loading labels…"
                      color: Qt.darker(root.contentForeground, 1.4)
                      font.family: root.contentFontFamily
                      font.pixelSize: Style.font.caption
                  ...
                  Row {
                    ...
                    NavButton {
                      id: aeSaveButton
                      text: root.aeSaving ? "Saving…" : "Save"
                      enabled: !root.aeSaving
                      onClicked: root.commitAdvancedEdit()
                    }
                    NavButton {
                      id: aeCancelButton
                      text: "Cancel"
          ...
          Item {
            anchors.fill: parent
            focus: root.advancedEditTaskId !== ""
            Keys.onPressed: function(event) {
              if (event.key === Qt.Key_Escape) { root.cancelAdvancedEdit(); event.accepted = true }
              else if (event.key === Qt.Key_Tab) { root.moveAdvancedFocus(1); event.accepted = true }
              else if (event.key === Qt.Key_Backtab) { root.moveAdvancedFocus(-1); event.accepted = true }
              else if (event.key === Qt.Key_Return || event.key === Qt.Key_Enter || event.key === Qt.Key_Space) {
                root.activateFocusedAdvancedControl(); event.accepted = true
              }
            }
          }
        }
